...
def main():
    ...
    menu = ["Login", "Signup", "Weather Alerts", "Discussion Forum","Farmer Chatbot"]
       ...
       if "user" in st.session_state:
        ...
        # Prefill city if available
          if username and users.get(username, {}).get("default_city"):
           ...
           # Always show weather UI
            weather_alerts_ui()
       else:
          st.warning("Please login to access Weather Alerts.")

    elif choice == "Farmer Chatbot":
        if "user" in st.session_state:
           farmer_chatbot_ui()
        else:
